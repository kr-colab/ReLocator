...
class NormalizationParams:
    ...
    def reverse(self, normalized_locs: np.ndarray) -> np.ndarray:
        ...
        return np.array([
            [x[0] * self.sdlong + self.meanlong,
             x[1] * self.sdlat + self.meanlat]
            for x in normalized_locs
        ])


def normalize_locs(locs: np.ndarray) -> Tuple[float, float, float, float, np.ndarray, np.ndarray]:
...
def filter_snps(genotypes, 
                ...
                impute: bool = False, 
                verbose: bool = False) -> Tuple[np.ndarray, FilterStats]:
...
def filter_snps_legacy(genotypes, 
                       min_mac: int = 1, 
                       max_snps: Optional[int] = None, 
                       impute: bool = False, 
