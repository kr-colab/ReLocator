...
class IndexSet:
    ...
    def __post_init__(self):
        ...
        if max_idx >= self.total_samples:
            raise ValueError(f"Index {max_idx} exceeds total_samples {self.total_samples}")
    ...
    def hold(self) -> np.ndarray:
        ...
        return self.indices.get('hold', self.indices.get('test', np.array([], dtype=int)))
    
    def get_split(self, name: str) -> np.ndarray:
        """Get indices for a named split."""
        if name not in self.indices:
            raise KeyError(f"Split '{name}' not found. Available splits: {list(self.indices.keys())}")
    ...
    def random_split(cls, 
                     ...
                     na_mask: Optional[np.ndarray] = None,
                     na_action: str = 'separate') -> IndexSet:
        ...
        if na_mask is not None:
            if na_action == 'fail' and np.any(na_mask):
                raise ValueError("Samples without coordinates found but na_action='fail'")
    ...
    def from_k_fold(cls, 
                    ...
                    fold: int,
                    seed: Optional[int] = None,
                    na_mask: Optional[np.ndarray] = None) -> IndexSet:
    ...
    def from_manual(cls,
                    ...
                    total_samples: Optional[int] = None) -> IndexSet:
