    ...
    if verbose:
        ...
        with tqdm(total=k, desc="Folds completed") as pbar:
            while futures:
                ...
                # Update progress bar
                pbar.set_postfix_str(f"Last: Fold {result['fold']}, GPU {result['gpu_id']}")
                pbar.update(1)
    else:
    ...
    if verbose:
        ...
        with tqdm(total=n_reps, desc="Replicates completed") as pbar:
            while futures:
                ...
                # Update progress bar
                pbar.set_postfix_str(f"Last: Rep {result['rep']}, GPU {result['gpu_id']}")
                pbar.update(1)
    else:
        # No progress bar if not verbose
        results = ray.get(futures)
...
def _create_ray_windows_worker(gpu_fraction: float = 1.0):
    ...
    def _ray_windows_worker(
        ...
        print(f"Worker processing window {window_idx} ({window_start}-{window_stop}) on GPU {gpu_id}")
...
def parallel_windows_holdouts(
    ...
    if not hasattr(locator, "positions") or locator.positions is None:
        if hasattr(locator, "_genotype_df"):
            ...
            if vcf is not None and "variants/POS" in vcf:
                ...
                raise ValueError(f"Could not load positions from VCF: {locator.config['vcf']}")
        else:
            raise ValueError(
    ...
    from locator.data.filters import normalize_locs
    meanlong, sdlong, meanlat, sdlat, unnormedlocs, normalized_locs = normalize_locs(locs)
    ...
    if verbose:
        print(f"Running windowed analysis for {len(windows)} windows across GPUs {gpu_ids} using Ray...")
    ...
    if verbose:
        ...
        for gpu_id in sorted(gpu_counts.keys()):
            print(f"  GPU {gpu_id}: {gpu_counts[gpu_id]} windows ({gpu_counts[gpu_id]/len(windows)*100:.1f}%)")
