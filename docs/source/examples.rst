...
Excluding Outliers After Analysis
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: python
    ...
    # Run holdout analysis to identify outliers
    loc.run_holdouts(genotypes, samples, k=50, replicates=10)

    # Load predictions and calculate errors
    predictions = pd.read_csv("qc_analysis_holdouts.txt", sep="\t")

    # Plot error summary and get merged data
    merged = plot_error_summary(
        predictions,
        "samples.txt",
        out_prefix="qc_analysis",
        return_merged=True
    )

    # Exclude samples with high prediction error
    loc.exclude_samples_by_condition(
        lambda df: df['error'] > 100,  # Error threshold in km
        sample_df=merged,
        reason="high_prediction_error"
    )

    # View excluded samples
    print(f"Excluded {len(loc.get_excluded_samples())} samples:")
    print(loc.get_excluded_samples())
...
.. code-block:: python
    ...
    # Create exclusion list from multiple quality metrics
    exclusion_reasons = {}

    # 1. Exclude samples with low genotype rate
    # Calculate genotype rates (proportion of non-missing data per sample)
    genotype_rates = []
    for i, sample_id in enumerate(samples):
        # For GenotypeArray, count non-missing calls
        if hasattr(genotypes, 'count_alleles'):
            # scikit-allel GenotypeArray
            sample_genos = genotypes[:, i, :]
            n_called = np.sum(sample_genos >= 0)
            n_total = sample_genos.size
        else:
            # NumPy array
            sample_genos = genotypes[:, i]
            n_called = np.sum(~np.isnan(sample_genos))
            n_total = len(sample_genos)

        rate = n_called / n_total if n_total > 0 else 0
        genotype_rates.append({'sampleID': sample_id, 'rate': rate})

    genotype_rates = pd.DataFrame(genotype_rates)
    low_quality = genotype_rates[genotype_rates['rate'] < 0.8]['sampleID'].tolist()
    for sample in low_quality:
        exclusion_reasons[sample] = "low_genotype_rate"

    # 2. Exclude geographic outliers (samples far from others)
    from scipy.spatial.distance import cdist

    # Load sample coordinates
    sample_data = pd.read_csv("samples.txt", sep="\t")
    coords = sample_data[['x', 'y']].values
    distances = cdist(coords, coords)
    mean_distances = distances.mean(axis=1)
    outlier_threshold = mean_distances.mean() + 3 * mean_distances.std()
    geo_outliers = sample_data[mean_distances > outlier_threshold]['sampleID'].tolist()
    for sample in geo_outliers:
        exclusion_reasons[sample] = "geographic_outlier"

    # 3. Create exclusion file
    with open("samples_to_exclude.txt", "w") as f:
        f.write("# Sample exclusion list generated by QC pipeline\n")
        f.write(f"# Generated on {datetime.now()}\n\n")
        for sample, reason in exclusion_reasons.items():
            f.write(f"# {reason}\n")
            f.write(f"{sample}\n")

    # Initialize Locator with exclusions
    loc = Locator({
        "exclude_samples": "samples_to_exclude.txt",
        "sample_data": "samples.txt",
        "out": "qc_filtered_analysis"
    })

    # Check data to see exclusion summary
    loc.check_data(genotypes, samples)
