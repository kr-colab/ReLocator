...
parser = argparse.ArgumentParser(
    description="run locator on a phased VCF, returning\
...
def sort_samples(samples):
    ...
    if not all(
        [sample_data["sampleID3"][x] == samples[x] for x in range(len(samples))]
...
def load_callbacks(boot):
    if args.bootstrap in ["True", "true", "TRUE", "t", "T"] or args.jacknife in [
        "True",
        "true",
        "TRUE",
        "t",
...
def predict_locs(
    ...
    if args.mode == "cv":
        ...
        mean_dist = np.mean(
            [
                spatial.distance.euclidean(p2[x, :], testlocs2[x, :])
    ...
    hist.to_csv(
        args.out + "_history.txt", sep="\t", index=False
    )  # TODO: add if/else for bootstraps?
...
if args.bootstrap in ["False", "FALSE", "F", "false", "f"] and args.jacknife in [
    "False",
    "FALSE",
    "F",
    "false",
    ...
    dists = predict_locs(
        model, predgen, sdlong, meanlong, sdlat, meanlat, testlocs, pred, samples
    )
...
elif args.bootstrap in ["True", "TRUE", "T", "true", "t"] and args.jacknife in [
    "False",
    "FALSE",
    "F",
    "false",
    ...
    for boot in range(args.nboots):
        ...
        dists = predict_locs(
            model, predgen2, sdlong, meanlong, sdlat, meanlat, testlocs, pred, samples
        )
        plot_history(history, dists)
        if args.keep_weights in ["False", "F", "FALSE", "f", "false"]:
            subprocess.run(
