...
ho_preds = locator.run_k_fold_holdouts(
    ...
    samples, 
    k=k, 
    verbose=True,  # Progress bar will still show
    return_df=True
)
...
ho_preds_merged['error_km'] = np.sqrt(
...
print(worst_predictions[['sampleID', 'error_km', 'x_true', 'y_true', 'x_pred', 'y_pred']])
...
worst_indices = [sample_list.index(sid) for sid in worst_sample_ids if sid in sample_list]
...
if hasattr(locator, 'positions') or locator.config.get('vcf'):
    ...
    if window_results is not None:
        ...
        window_cols = [col for col in window_results.columns if col.startswith(('x_pos', 'y_pos'))]
        ...
        for i, sample_id in enumerate(worst_sample_ids[:6]):
            if sample_id in window_results['sampleID'].values:
                sample_window_data = window_results[window_results['sampleID'] == sample_id]
                ...
                distances = [np.sqrt((x - mean_x)**2 + (y - mean_y)**2) * 111.32 
                           for x, y in zip(x_preds, y_preds)]
                
                window_positions = [int(col.split('x_pos')[1]) / 1e6 for col in x_window_cols]  # Convert to Mb
                
                # Get error for coloring
                error_km = worst_predictions[worst_predictions['sampleID'] == sample_id]['error_km'].values[0]
                
                ax1.plot(window_positions, distances, 'o-', 
        ...
        for sample_id in worst_sample_ids[:6]:
            if sample_id in window_results['sampleID'].values:
                sample_window_data = window_results[window_results['sampleID'] == sample_id]
                true_sample = worst_predictions[worst_predictions['sampleID'] == sample_id].iloc[0]
                ...
                for x_col, y_col in zip(x_window_cols, y_window_cols):
                    ...
                    error_km = np.sqrt((x_pred - true_sample['x_true'])**2 + 
                                     (y_pred - true_sample['y_true'])**2) * 111.32
        ...
        if error_matrix:
            ...
            im = ax2.imshow(error_matrix, aspect='auto', cmap='YlOrRd', interpolation='nearest')
            
            # Set ticks
            ax2.set_xticks(range(len(x_window_cols)))
            ax2.set_xticklabels([f'{int(col.split("x_pos")[1])/1e6:.1f}' 
                                for col in x_window_cols], rotation=45)
            ...
            for i in range(len(error_matrix)):
                ...
                ax2.text(best_window_idx, i, '★', ha='center', va='center', 
