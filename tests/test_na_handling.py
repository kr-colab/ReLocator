...
class TestNAHandling:
    ...
    def test_check_data_basic(self):
        ...
        sample_df = pd.DataFrame(
            {"sampleID": samples, "x": [1.0, np.nan, 3.0], "y": [4.0, np.nan, 6.0]}
        )
    ...
    def test_check_data_output_exclude_mode(self, capsys):
        ...
        sample_df = pd.DataFrame(
            {"sampleID": samples, "x": [1.0, np.nan], "y": [2.0, np.nan]}
        )
    ...
    def test_check_data_many_na_samples(self, capsys):
        ...
        sample_df = pd.DataFrame({"sampleID": sample_ids, "x": x_vals, "y": y_vals})
...
class TestPhase2NAHandling:
    ...
    def test_train_method_override(self, capsys, tmp_path):
        ...
        history = locator.train(
            genotypes=genotypes, samples=samples, na_action="separate"
        )
    ...
    def test_run_bootstraps_with_na_action(self, capsys, tmp_path):
        ...
        result = locator.run_bootstraps(
            genotypes=genotypes, samples=samples, n_bootstraps=2, return_df=True
        )

        captured = capsys.readouterr()
        assert (
            "Bootstrap analysis: 7 samples with coordinates, 3 without" in captured.out
        )
        assert "NA handling mode: separate" in captured.out
...
class TestPhase3NAHandling:
    ...
    def test_run_k_fold_holdouts_with_na_action(self, capsys, tmp_path):
        ...
        result = locator.run_k_fold_holdouts(
            genotypes=genotypes, samples=samples, k=2, return_df=True, verbose=True
        )
    ...
    def test_run_jacknife_holdouts_with_na_action(self, capsys, tmp_path):
        ...
        captured = capsys.readouterr()
        assert (
            "Jacknife holdout analysis: 7 samples with coordinates, 3 without"
            in captured.out
        )
        assert result is not None
        assert isinstance(result, pd.DataFrame)
    ...
    def test_run_windows_holdouts_with_na_action(self, capsys, tmp_path):
        ...
        geno_df = pd.DataFrame(allele_counts.T, index=samples, columns=positions)
        ...
        captured = capsys.readouterr()
        assert (
            "Windows holdout analysis: 7 samples with coordinates, 3 without"
            in captured.out
        )
        assert "Note: Holdout analysis requires known locations" in captured.out
        assert result is not None
