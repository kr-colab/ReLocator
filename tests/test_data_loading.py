...
def test_load_genotypes_from_dataframe():
    ...
    np.testing.assert_array_equal(samples, np.array(["sample1", "sample2", "sample3"], dtype=object))
    
    # Verify positions were stored
    assert hasattr(locator, "positions")
    np.testing.assert_array_equal(locator.positions, np.array([1001, 2005, 3010], dtype=float))
...
def test_load_genotypes_from_matrix_file():
    ...
    try:
        ...
        assert isinstance(genotypes, allel.GenotypeArray)
        assert genotypes.shape == (3, 3, 2)  # 3 SNPs, 3 samples, diploid
        np.testing.assert_array_equal(samples, np.array(["sample1", "sample2", "sample3"]))
