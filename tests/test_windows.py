...
class TestWindowAnalysis:
    ...
    def create_test_data_with_positions(self, n_samples=15, n_snps=100, n_known=12):
        ...
        geno_df = pd.DataFrame(ac.T, index=samples, columns=positions)
    ...
    def test_run_windows_basic(self, tmp_path):
        ...
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions()
        )
    ...
    def test_run_windows_with_na_samples(self, tmp_path):
        ...
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions(n_samples=20, n_snps=100, n_known=15)
        )
    ...
    def test_run_windows_window_size(self, tmp_path):
        ...
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions(n_samples=10, n_snps=150, n_known=10)
        )
    ...
    def test_run_windows_holdouts(self, tmp_path):
        ...
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions(
                n_samples=15,
                n_snps=100,
                n_known=15,  # All samples need coordinates for holdout
            )
        )
    ...
    def test_window_analysis_without_positions(self, tmp_path):
        ...
        sample_df = pd.DataFrame(
            {"sampleID": samples, "x": [1.0, 2.0, 3.0], "y": [4.0, 5.0, 6.0]}
        )
        ...
        with pytest.raises(ValueError, match="SNP positions required"):
            locator.run_windows(
                genotypes=genotypes, samples=samples, window_size=100_000, return_df=True
            )

    def test_window_start_stop(self, tmp_path):
        """Test window start and stop parameters."""
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions(n_samples=10, n_snps=100, n_known=10)
        )
    ...
    def test_run_windows_holdouts_with_indices(self, tmp_path):
        ...
        genotypes, samples, sample_df, geno_df, positions = (
            self.create_test_data_with_positions(n_samples=15, n_snps=100, n_known=15)
        )
    ...
    def test_run_windows_holdouts_empty_windows(self, tmp_path):
        ...
        genotype_array = np.random.randint(
            0, 2, size=(n_snps, n_samples, 2), dtype=np.int8
        )
        genotypes = allel.GenotypeArray(genotype_array)

        # Create sample data
        sample_df = pd.DataFrame(
            {"sampleID": samples, "x": range(n_samples), "y": range(10, 10 + n_samples)}
        )
