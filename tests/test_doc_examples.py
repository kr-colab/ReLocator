...
from locator import Locator, EnsembleLocator
from locator.data import IndexSet, filter_snps, normalize_locs, make_tf_dataset
from locator.plotting import plot_predictions, plot_error_summary, plot_sample_weights
...
class TestDataPipelineExamples:
    ...
    def test_indexset_usage(self, sample_data):
        ...
        assert len(index_set.train) + len(index_set.val) + len(index_set.test) == n_samples
    ...
    def test_normalize_locs_example(self, sample_data):
        ...
        meanlong, sdlong, meanlat, sdlat, unnormed_locs, normed_locs = normalize_locs(valid_coords)
...
class TestPlottingExamples:
    ...
    def test_plot_sample_weights(self, sample_data, temp_dir):
        ...
        plot_sample_weights(
            loc,
            os.path.join(temp_dir, "sample_weights"),
            plot_map=False,
            show=False
        )
        
        assert os.path.exists(os.path.join(temp_dir, "sample_weights_sample_weights.png"))
    ...
    def test_parallel_kfold(self, sample_data, temp_dir):
        ...
        try:
            ...
            predictions = parallel_k_fold_holdouts(
                loc, genotypes_valid, samples_valid,
                k=3,
